...
description: 专业旅行攻略生成 Skill（人称「小泷」）。输入目的地/天数/人数/预算/节奏/美食/打卡等需求，联网检索真实数据，生成含 9 固定章节 + **购物和伴手礼第 10 节（默认包含）** 的可执行攻略，强制逐小时时间线 + 每日 Google 地图路线 + 就近用餐铁律 + 提前购票官网链接 + 8–10 条实用贴士。所有具体店铺与景点 / 游乐项目写入前必须经 WebSearch + 官方源核实 2026 仍在营业 / 当前开放运营；默认应用用户长期偏好（不吃奇形怪状食材、不排网红店、少走路多坐公交、出片打卡）。
---
...
# ⚠️ 四条核心铁律（生成前必读对应资产文件）
1. **店铺营业状态核实** → 读 `assets/research_checklist.md`「店铺」小节。任何餐厅/甜点店/酒店写入前必须 WebSearch+Google Maps 核实 2026 仍在营业，停业店用同区域同类型替代。可用 `scripts/verify_shops.py` 批量生成查询。
...
4. **景点 / 游乐项目运营状态核实** → 读 `assets/research_checklist.md`「景点 / 游乐项目」小节。任何**景点、博物馆、观景台、游乐设施、演出、巡游、烟花、缆车**写入攻略前，必须用 WebSearch + 官方网站 / 官方 APP 核实**当前是否开放运营**（主题公园的设施/演出常有季节性检修、翻新关闭）。已关闭/停运的项目**不得写入必玩**，改为同区域可替代项目；并在行程 / 第 8 节注明核实日期。核实方法与店铺完全一致，可复用 `scripts/verify_shops.py` 批量出查询（传入景点/设施名列表即可）。

# 调用流程（每次生成攻略）
1. **读资产**：`assets/user_preferences.md`（硬约束）+ `assets/output_structure.md`（格式）+ `assets/shop_lists.md`（已核实店铺库）。
2. **研究**：WebSearch 抓真实数据（交通/票价/门票/营业）；对 `shop_lists.md` 候选店**快速复检**营业状态，新店用 `scripts/verify_shops.py` 批量出查询逐项核实；**对第 2 节每个景点 / 游乐设施 / 演出 / 巡游 / 烟花**用 WebSearch + 官网 / 官方 APP 核实**当前开放运营状态**（尤其迪士尼等主题公园须查当日），已关闭的替换或剔除。
...
5. **自查**：跑 `research_checklist.md` 末尾清单；确认无 "permanently closed/temporarily closed/已停业/已关闭/停运" 字样的店铺或景点被写进攻略，9 节齐全，签名到位。
